<script lang="ts">
	 ...
	 *
	 * English only, as it was on its own page: the words are about an
	 * authenticator app and recovery codes, and an admin reads them in the
	 * console, which has no language of its own.
	 */
	type Props = {
		data: {
			email: string;
...
{#if data.required && !data.state.verified}
	...
	>
		This address administers the instance, so it needs a second factor before the admin console will
		open.
	</p>
{/if}
...
{#if setup}
	...
	<section class="mt-8" aria-labelledby="codes">
		...
		<p class="text-fg-secondary mt-2">
			These are shown once and never again. Each one works a single time, and they are the only way
			back in if you lose the app. Write them down somewhere that is not your phone.
		</p>
		<ul
