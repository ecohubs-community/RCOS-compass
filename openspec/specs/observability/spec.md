...
## Purpose
Covers what an operator can see: the scrubbed, grouped error record kept on this instance rather than sent to a third party, the mail failure recorded without the recipient's address, the seven onboarding milestones counted per community and never per person, and what the status page must show for somebody to answer "is anything broken right now".
## Requirements
