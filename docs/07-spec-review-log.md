...
| 19 | L | Standard content licensing/attribution in exports is unstated. | Add a licence line to `standard/` and to every export. | **shipped P7** — from `meta.yaml`'s own fields, in the export readme and manifest, on the public pages and in both footers, with PolyForm NC named beside CC BY 4.0. A standard published under other terms carries those instead of a hard-coded string |
...
| 23 | H | **Append-only register vs right to erasure.** Both are real obligations and they collide. | Decisions reference `membership_id`, never a denormalised name; erasure tombstones the profile and renders `Former member (M-0142)`; the governance record survives; corrections are new entries. Must be stated verbatim in the privacy policy. | **shipped P7** — and the design was one step short: a rendering function reaches surfaces that read a row, not the four places a name was *copied* when it was written (`audit_event.actor_email`, `invitation.email`, `decision_attendee.external_name`, and git commit bodies, which are beyond any erasure). Correction alone does not answer a request either; redaction does, over any body a member wrote. The policy states it verbatim and a test asserts the policy against the code |
...
| 32 | M | **Accessibility risk baked into the design tokens**: muted `#6B7574` on `#0E1011` is ~4.1:1, used at 11px in the sidebar and table footers — below AA. Dark-only with no theme escape hatch compounds it. | Lift muted or raise those sizes; every colour a token so a high-contrast theme is a token file; axe pass on every route. | **closed P7** — the lift happened in P0 and nothing checked it. `tests/unit/contrast.test.ts` now asserts twelve pairs against `app.css` at the size each is used, and the axe pass is driven from a route registry, so a screen added later is scanned or exempt with a reason |
