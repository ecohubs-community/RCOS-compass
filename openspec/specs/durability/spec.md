...
## Purpose
Covers getting a community's work back: the snapshot that takes the uploaded files with the database because a restore missing either half is broken, the restore that refuses to run over a live instance or from an incomplete snapshot, and the drill that proves it by re-opening a mapped document rather than describing the procedure.
## Requirements
