...
## Purpose
Covers the Documents library: uploading several files at once with the member-visibility notice, the one mapping state every document shows and how it is derived, filters kept in the URL, the next action a row offers, and the menu that replaces a file, restores an earlier version or removes a document.
## Requirements
