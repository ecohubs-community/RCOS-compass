...
## Purpose
Covers the documents a community reads before it agrees to anything: the privacy policy, pilot terms and sub-processor list served at stable anonymous paths; the review recorded against the exact wording so an edit re-arms the draft notice by itself; the data inventory the policy is written from and the test that keeps it complete; and the licence and attribution that travel with published and exported content.
## Requirements
