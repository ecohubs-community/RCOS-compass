# document-scan Specification

## Purpose
Covers the AI scan of a document: started by a member and never by an upload, run as resumable batches charged to whoever starts or continues it, claimed so only one scan of a document is live, never writing into content that changed under it, never paying twice for a paragraph, and never left scanning forever.
## Requirements
