# git-mirror Specification

## Purpose
Covers verifiable history a community can walk away with: the git repository every community gets with no configuration, one commit per decision carrying the rendered artifact and the decision record, downloadable as a bundle that clones with git alone; the optional remote a community links, pushed after the freeze rather than inside it and never forced; and the credential, which is encrypted at rest, write-only from the interface, and absent from every log line, error body, export and admin screen.
## Requirements
