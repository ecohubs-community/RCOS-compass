...
- [ ] 2.1 better-auth wired to Drizzle, email+password and magic link, verification required
- [ ] 2.2 Session cookie hardening and absolute lifetime; server-side revocation
...
- [ ] 2.5 Tests: unverified session is withheld from communities; revoked session refused; expired session refused; no user enumeration on sign-in or reset; admin without TOTP cannot reach admin routes
...
- [ ] 3.3 `ctx` object threaded into services; no service signature accepts a community id from input
- [ ] 3.4 Tests: switching communities carries nothing over; membership removal and role change take effect on the next request; a suspended community refuses writes
...
- [ ] 5.1 A service registry, so the isolation suite enumerates services rather than trusting a list someone maintains
- [ ] 5.2 The parameterised cross-tenant suite: for every service, a B member with an A resource id gets nothing
- [ ] 5.3 The route guard sweep: every server file under `(app)` and `(admin)` calls `requirePermission`
- [ ] 5.4 Tests are the deliverable here; there is no feature code
