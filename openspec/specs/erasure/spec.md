...
## Purpose
Covers being forgotten without unmaking the record: what erasure removes and what it must not, the tombstone and the community-local `Former member (M-####)` produced by one function every surface goes through, the places a name was copied rather than rendered, correction versus redaction over any body a member wrote, and the refusals that stop an erasure stranding a community or an instance.
## Requirements
