...
## Purpose
Defines the tenant boundary: the community is resolved from the URL rather than the session, non-membership is indistinguishable from non-existence, every service filters by the resolved community, and membership changes take effect on the next request.
## Requirements
