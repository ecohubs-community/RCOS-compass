...
## Purpose
Covers a community taking everything it has and leaving: the Markdown, PDF and JSON bundle built as a background job, readable with no Compass, no network and no database, carrying only what its requester could see and saying which levels those were, labelling local additions as the community's own, produced from the same rendering as the public page and the mirror, and delivered through a signed link that expires and is audited at both ends.
## Requirements
